...
class EpidemicModel:
    ...
    def __init__(self, args, env_count=20):
        ...
        # 每个区域单步的观测维度
        self.local_obs_dim = 1 if "local_obs_dim" not in args_dict else args.local_obs_dim
        ...
        self.long_term_reward_weight = 0 if "long_term_reward_weight" not in args_dict else args.long_term_reward_weight

        """ 不确定性1：不完全观测 """
        # TODO: 观测的不完全通过detect_E_rate和detect_I_rate进行控制？ 目前是根据上报率做的不完全观测
        # 是否对新增I进行不完全观测
        ...
        self.obs_imperfect_down = 0.3 if "obs_imperfect_down" not in args_dict else args.obs_imperfect_down
    ...
    def reset(self, rand_idxs=None):
        ...
        # TODO: 需要重建的信息2：daily_new_total_EI 记录真实新增，是需要重建信息的一部分；需要重建的信息1：现存EI
        self.daily_new_total_EI = torch.zeros((self.env_count, self.period + 1, self.ZONE_NUM), device=self.device)
    ...
    def step(self, action = None):
        ...
        # 记录动作
        # a_q = torch.ones_like(action) * 4
        # action = torch.cat((action, a_q), dim=-1)
        if action is None:
        ...
        # 1.1.状态转移-检测
        # 检测的总人数 / 待检测的人数
        # test_num_rate = actual_p_test * self.POP / (self.simState[:, :, [self.S, self.E_undetected, self.I_undetected, self.R]].sum(dim=-1)
        #                                             - self.daily_known2R.sum(dim=1) + 1e-12)
        ...
        # 1.2.状态转移-隔离
        # 隔离的总人数 / 待隔离的人数
        # quara_num_rate = actual_p_quara * self.POP / (self.simState[:, :, self.can_isolated].sum(dim=-1) + 1e-12)
        ...
        # 更新状态
        dState = torch.stack([dS, dE_undetected, dE_detected, dI_undetected, dI_detected, dI_reported, dQE, dQI, dR], dim=2) # (env_count, ZONE_NUM, 9)

        # 计算新增后再更新状态
        # 将新增的E返回给各个区域
        S_toJ = torch.bmm(self.OD.transpose(2, 1), self.simState[:, :, self.S].unsqueeze(-1)).squeeze(-1)  # (env_count, ZONE_NUM)
        new_E_inJ = S_toJ * modified_ratio  # (env_count, ZONE_NUM)
        ...
        # self.daily_new_E_self_cause[:, self.day - 1, :] = self._process_less_than_one(new_E_backI)

        # 计算新增的E I
        self.daily_new_E[:, self.day - 1, :] = S2Eun  # (env_count, zone_num)
        self.daily_new_I[:, self.day - 1, :] = Eun2Iun + Ede2Ide + QE2QI  # (env_count, zone_num)
        self.daily_new_total_EI[:, self.day - 1, :] = S2Eun + Eun2Iun + Ede2Ide # TODO: 用这个作为重建目标是否合适？
        ...
        # self.daily_new_Q[:, self.day - 1, :] = self._process_less_than_one(self.daily_new_Q[:, self.day - 1, :])
        # self.daily_detect_EI[:, self.day - 1, :] = self._process_less_than_one(self.daily_detect_EI[:, self.day - 1, :])
        # self.daily_known2R[:, self.day - 1, :] = self._process_less_than_one(self.daily_known2R[:, self.day - 1, :])
        ...
        if self.day > self.period:
            ...
            score = torch.exp((20 * total_infections) / self.TOTAL_POP) + torch.exp((total_quarantine + self.reward_weights[1] * total_test_num) / self.TOTAL_POP) # (env_count)
            print("reward:", ep_r.mean(), "total_infections:", total_infections.mean(), "total_test_num:", total_test_num.mean(),
                  "total_quarantine:", total_quarantine.mean(), "score:", score.mean())
    ...
    def _random_mask(self):
        rate = torch.rand(self.env_count, device=self.device) * 0.2 + 0.3 # 0.2-0.6之间的随机数
        mask = torch.rand(self.env_count, self.ZONE_NUM, device=self.device) < rate.unsqueeze(-1).expand(-1, self.ZONE_NUM)
        cds = torch.randint(14, 21, (self.env_count, 1), device=self.device).squeeze(-1)
        ...
        self.count_downs -= 1
        # rate = torch.rand(1, device=self.device) * 0.5
        # if not hasattr(self, 'old_zone_mask') or self.count_down == 0:
        #     mask = torch.rand(self.env_count, self.ZONE_NUM) < rate
        #     self.old_zone_mask = mask
        #     self.count_down = random.randint(7, 14)
        # self.count_down -= 1
    ...
    def _action_to_u(self, action):
        ...
        u_p_test = self.action_to_u0[(action / u1_cnt).to(torch.long)] # (env_count, ZONE_NUM)
        u_p_quara = self.action_to_u1[(action % u1_cnt).to(torch.long)] # (env_count, ZONE_NUM)
    ...
    def _get_obs(self):
        ...
        # 从检测的EI估计出Eun Iun
        p_test, p_quara = self._action_to_u(self.actions[:, self.day - 2, :])  # (env_count, ZONE_NUM)
        mask = p_test != 0
        estimate_Eun, estimate_Iun = torch.zeros_like(p_test), torch.zeros_like(p_test)
        estimate_Eun[mask] = self.daily_detect_E[:, self.day - 1, :][mask] / self.detect_E_rate / p_test[mask]
        estimate_Iun[mask] = self.daily_detect_I[:, self.day - 1, :][mask] / self.detect_I_rate / p_test[mask]

        curr_obs = [curr_known_EI + estimate_Eun + estimate_Iun, estimate_Eun] # [现存的，新检测出的E]
    ...
    def _reward_func(self, actual_p_test, actual_p_quara):
        ...
        local_infe_cost = (20 * new_E_self_cause) ** 1 / self.POP # 即时奖励
        global_infe_cost = (20 * new_E_self_cause.sum(dim=1)) ** 1 / self.TOTAL_POP # 全局奖励


        # 2.检测成本 actual_p_test
        state = self.simRes[:, self.day - 2, :, :] # 检测的成本是针对前一天的
        # test_num = actual_p_test * self.POP # (env_count, zone_num)
        test_num = actual_p_test * (state[:, :, [self.S, self.E_undetected, self.I_undetected, self.R]].sum(dim=-1) - self.daily_known2R.sum(dim=1))
        self.daily_test_num[:, self.day - 1, :] = test_num
        avg_test_num = self.daily_test_num.sum(dim=1) / (self.day - 1)  # (env_count, zone_num)
        # test_num = avg_test_num
        ...
        quara_num = torch.max(quara_num, actual_p_quara * 10)
        self.daily_quara_num[:, self.day - 1, :] = quara_num
        avg_quarantine = self.daily_quara_num.sum(dim=1) / (self.day - 1)  # (env_count, zone_num)
        # new_q = avg_quarantine
        ...
        # print(local_infe_cost.mean(), local_test_cost.mean(), local_quara_cost.mean())
        ...
        # 计算一次score
        # total_infections = self.daily_new_I.sum(dim=1)  # (env_count, zone_num)
        # total_test_num = self.daily_test_num.sum(dim=1)  # (env_count, zone_num)
        # total_quarantine = self.daily_quara_num.sum(dim=1)  # (env_count, zone_num)
        # # total_infections = self.daily_new_I[:, self.day - 1, :]  # (env_count, zone_num)
        # # total_test_num = self.daily_test_num[:, self.day - 1, :]  # (env_count, zone_num)
        # # total_quarantine = self.daily_quara_num[:, self.day - 1, :]  # (env_count, zone_num)
        # # local_score = torch.exp((20 * total_infections) / self.POP) + torch.exp(
        # #     (rw[1] * total_test_num + rw[2] * total_quarantine) / self.POP)  # (env_count, zone_num)
        # global_score = torch.exp((20 * total_infections.sum(dim=-1)) / self.TOTAL_POP) + torch.exp(
        #     (rw[1] * total_test_num.sum(dim=-1) + total_quarantine.sum(dim=-1)) / self.TOTAL_POP)  # (env_count)
        #
        # reward = (1 - self.long_term_reward_weight) * reward + self.long_term_reward_weight * (-global_score).unsqueeze(-1)
        ...
        if self.use_reward_shaping:
            ...
            # action = self.actions[:, self.day - 2, :]
            # action_small = action[:, :, 0].float() < 1.0 # (env_count, zone_num)
            # zone_mask = infections_large & action_small
            # 如果感染人数没有，但采取较强的控制措施
            infections_small = infections < 0.01
            action_large = actual_p_test >= 0.5
        ...
        self.history_cost['quara_num'][:, self.day - 2] = state[:, :, [self.QE,self.QI]].sum(dim=[1,2])
    ...
    def __cal_pearsonr(self):
        mask = torch.rand(643) < 0.3  # 小于比例的为 True
    ...
    def render(self, title='', fig_dir=None) :
        ...
        plt.plot(estimate_total_EI.cpu(), label='estimate_total_EI (avg)', color='red', linewidth=2, linestyle='--', alpha=0.5)
        # 观测的新增EI
        estimate_new_E = (self.history_local_obs[:, :, :, 1] * self.POP.unsqueeze(1)).sum(dim=2).mean(dim=0)
        plt.plot(estimate_new_E.cpu(), label='estimate_new_E (avg)', color='green', linewidth=2, linestyle='--', alpha=0.5)
        ...
        plt.plot(total_EI.cpu(), label='Total EI (avg)', color='red', linewidth=2, alpha=0.5)
        # 实际的新增EI
        ...
        quara_num = self.history_cost['quara_num'].mean(dim=0)
