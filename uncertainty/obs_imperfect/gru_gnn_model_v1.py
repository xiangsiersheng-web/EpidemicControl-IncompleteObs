...
class GraphConvolution(nn.Module):
    ...
    def forward(self, x, OD, POP=None, obs_mask=None):
        ...
        if POP is None:
            # # TODO：改用标准的GCN
            # adj = OD[0] + torch.eye(OD[0].size(0)).to(OD.device)  # 添加自环
            # deg = adj.sum(dim=1).pow(-0.5)
            # norm_adj = deg.unsqueeze(1) * adj * deg.unsqueeze(0)  # 对称归一化
            # norm_adj = norm_adj.unsqueeze(0).expand(x.shape[0], -1, -1)
            #
            # # 特征变换 → 聚合 → 激活
            # x = self.mlp(x)  # 特征变换
            # x = torch.bmm(norm_adj, x)  # 邻域聚合
            # x = F.relu(x)  # 非线性激活
