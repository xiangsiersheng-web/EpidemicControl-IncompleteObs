...
def plot_reward_comparison(all_rewards, alpha=0.3, save_fig=True,
    ...
    for drd_label in ['有责任分发', '无责任分发']:
        ...
        label = '有责任分配机制' if drd_label == '有责任分发' else '无责任分配机制'
    ...
    # plt.xlabel('Step')
    # plt.ylabel('Reward')
    plt.xlabel('步数', fontname='SimSun')
    plt.ylabel('奖励', fontname='SimSun')
    ...
    for text in legend.get_texts():
        text.set_fontfamily('SimSun')  # SimSun是宋体的英文标识
...
if __name__ == '__main__':
    ...
    fig_path = "reward_data_high.png"
    plot_reward_comparison(all_rewards, fig_path=fig_path)