...
def _plot_three_level_compare(
    ...
    ax.plot(x, true_mean, label="完全可观测", linewidth=2, color="blue")

    # 完备上报：天蓝
    ax.fill_between(x, perf_p10, perf_p90, color="orange", alpha=0.25)
    ax.plot(x, perf_mean, label="稳态部分观测", linewidth=2.0, color="orange")

    # 部分上报：浅蓝（或婴儿蓝）
    ax.fill_between(x, part_p10, part_p90, color=light_blue, alpha=0.25)
    ax.plot(x, part_mean, label="非稳态部分观测", linewidth=2.0, color=light_blue)

    ax.set_xlabel("时间 (天)", fontproperties="SimSun")
    ax.set_ylabel(r"现存感染规模$I_{\mathbf{combined}}$ (人)", fontproperties="SimSun")
...
def plot_observation_levels_compare():
    ...
    _plot_three_level_compare(true_state_overall, perfect_obs_overall, partial_obs_overall,
                              title="Overall", save_path="figure/overall.png")
    # 绘制某一个区域的
    region_idx = 10
    true_state_region = true_state[:, :, region_idx]
    perfect_obs_region = perfect_obs[:, :, region_idx]
    partial_obs_region = partial_obs[:, :, region_idx]
    _plot_three_level_compare(true_state_region, perfect_obs_region, partial_obs_region,
                              title=f"Region {region_idx}", save_path=f"figure/region_{region_idx}.png")
    for idx in [359, 363]:
